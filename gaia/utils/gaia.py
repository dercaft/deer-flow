...
from gaia.benchmarks import BaseBenchmark
# from gaia.tasks import Task # TODO: Change to deer-flow
import logging
...
class GAIABenchmark(BaseBenchmark):
    ...
    async def run(
        ...
        for task in tqdm(datas, desc="Running"):
            if self._check_task_completed(task["task_id"]):
                logger.info(f"SUCCESS:The following task is already completed:\n task id: {task['task_id']}, question: {task['Question']}")
                continue
    ...
    def normalize_number_str(self, number_str: str) -> float:
        try:
            ...
            logger.error(f"Error in normalizing number string: {e}")
            return float("inf")
        try:
